...
- **Human-In-The-Loop (HITL)**: When either agent flags a critical assumption, LangGraph pauses execution (`waiting_user_validation`) and waits for human approval or rejection via the UI before resuming. Task status toggles (`todo` / `doing` / `done`) do not mark a node dirty.
...
That runs the planner consensus tests and a functional usability journey: edit a dirty Epic, attach a review note, and assert the Manager ↔ Architect loop uses it.
